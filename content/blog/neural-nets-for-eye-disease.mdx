...
![Figure 3 from the Ophthalytics paper: retinal fundus images](/images/blog/neural-nets-for-eye-disease/ophth-fig-3.png "(a) DR Negative and (b) DR Positive")
...
<Callout type="info" title="Update 3/31/25">
Since writing this, I've read much more literature on retinal fundus imaging and computer vision. The gold standard model architecture seems to be a U-Net with a pretrained encoder (ImageNet) for segmentation, connected to a pre-trained classifier (ImageNet). If I go any further with this project, this is the direction I'll take. Lack of annotated data is the biggest issue.
</Callout>

## The Code, Explained

<Callout type="note">
Again, [here is the link to the code](https://colab.research.google.com/drive/1u16VMEfoQmuXb5adNLExwes6OQZbT-7e?usp=sharing). I've set it up so that you should be able to download the data, train/fine-tune the model, and test it pretty easily. NOTE: You'll probably need an A100 at least, which means you need Colab Pro. You could probably get away with a T4 (free) for the smallest model, *EfficientNet-B0*. [Email me](mailto:ben@beneverman.com) if you have any issues or questions!
</Callout>
...
This will show you the GPU you're using. Here's what an example output looks like:
...
<Callout type="tip">
In Colab you can click the folder icon on the left to see the files in the runtime. Or, you can use the `!ls` command to list the files in the current directory and `cd` to change directories.
</Callout>
...
The output should be `['glaucoma', 'cataract', 'diabetic_retinopathy', 'normal']`
...
Okay, now let's preprocess the data into datasets. We'll do some minimal preprocessing before we split the data into train, validation, and test sets. Before we do this, it can be helpful to look at the data and see what we're working with.[^8]
...
def make_datasets(path: str):
    ...
    # time to split the data into train, validation, and test sets
...
Now we can make the datasets and dataloaders.[^9]
...
Lets define the loss function and optimizer. We're using the CrossEntropyLoss function, which is commonly used for multi-class classification problems. The optimizer is Adam, which is a popular choice for deep learning models.[^10]
...
4. Forward pass the inputs through the model `output = model(inputs)`
...
13. Important: I've added early stopping. This is a technique to prevent overfitting (the bane of every ML engineer's existence). If the validation loss doesn't improve for a certain number of epochs, we stop training as to not overfit the model.
...
<Callout type="error">
If you are NOT using a GPU runtime (`!nvidia-smi` should show a GPU), this will take a long time (hours or days) and might never finish. If you're on a CPU, you need to restart the runtime and select a GPU runtime, before working back to this point (you'll need to re-run all previous cells).
</Callout>
...
```python
visualize_predictions(model, test_loader, class_names=["cataract", "glaucoma", "diabetic_retinopathy", "normal"], num_images=8)
```
...
![Example model output: predicted vs true labels on test images](/images/blog/neural-nets-for-eye-disease/example-output.png "Model predictions on random test images — true label vs predicted")
...
[^8]: Andrej Karpathy (AKA the GOAT 🐐) has a [fantastic blog post](https://karpathy.github.io/2019/04/25/recipe/) that includes how and why you should manually inspect your data.
[^9]: Usually a batch size of 32 or 64 works best. [Here's a detailed explanation from Weights & Biases](https://wandb.ai/ayush-thakur/dl-question-bank/reports/What-s-the-Optimal-Batch-Size-to-Train-a-Neural-Network---VmlldzoyMDkyNDU)
[^10]: You can read more about how deep learning works in this fantastic book by [Francois Fleuret](https://fleuret.org/francois/), [*The Little Book of Deep Learning*](https://fleuret.org/public/lbdl.pdf). I would recommend trying to get the code to work first, and then trying to reverse engineer it and understand how/why it works.
